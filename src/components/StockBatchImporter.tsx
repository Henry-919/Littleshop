...
const compressImageDataUrl = (
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const jpegQuality = typeof options?.jpegQuality === 'number' ? options.jpegQuality : 0.72;
  ...
  const processImages = async (files: File[]) => {
    ...
    try {
      ...
      const callAnalyzeStockApi = async (payloadDataUrl: string, strategyLabel: string) => {
        ...
        throw new Error(`[${strategyLabel}] 图片识别失败`);
      };
  ...
  return (
    <>
      ...
      <button
        ...
        className="w-full sm:w-auto h-11 px-4 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-xl font-bold transition-all inline-flex items-center justify-center gap-2 border border-emerald-100 shadow-sm text-sm"
      >
