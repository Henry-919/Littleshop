...
export function ReceiptScanner({ store }: { store: any }) {
  ...
  const applySelectedManualSales = async () => {
    ...
    try {
      ...
      const nextFormData: Record<number, { name: string; costPrice: string; stock: string; sellingPrice: string }> = {};
      let newIdx = 0;
                ...
                {pendingAnalysis.previewItems.map((item, idx) => (
                  <div key={idx} className="bg-white rounded-lg border border-sky-100 p-2.5 text-xs">
                    ...
                    {item.hasMathDiscrepancy && (
                      <div className="text-amber-600 mt-1">?? 金额校验异常</div>
                    )}
                            ...
                            <div>
                              ...
                              <input
                                ...
                                onChange={(e) => setInventoryFormData(prev => ({
                                  ...prev,
                                  [idx]: { ...(prev[idx] || { name: '', costPrice: '', stock: '', sellingPrice: '' }), name: e.target.value }
                                }))}
